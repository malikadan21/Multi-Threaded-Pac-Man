...
const int NUM_RESOURCES = 1,WINDOW_WIDTH = 800,WINDOW_HEIGHT = 600,PACMAN_SIZE = 6,COIN_SIZE = 1,NUM_GHOSTS = 4,MAX_LIVES = 3;
const float PACMAN_SPEED = 2.0f,GHOST_SPEED = 1.5f;

bool movingLeft = false,movingRight = false,movingUp = false,movingDown = false,gamestarted = false,check=true,ghostpowerupenabled=false,energizereaten= false;
...
pthread_mutex_t keyMutex,permitMutex,ghostfastMutex;
pthread_mutex_t pacmanMutex,ghostMutex,scoreMutex,livesMutex;

pthread_t threadEnergizer;


enum Cell{ Door,Empty,Energizer,Pellet,Wall,Ghost,GhostHouse,Powerup };

struct Position { short x; short y;
bool operator==(const Position& i_position){	return this->x == i_position.x && this->y == i_position.y;}
};
...
const int MAP_WIDTH = 21,MAP_HEIGHT=21,CELL_SIZE=21;
